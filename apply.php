...
<div class="main-content">
    ...
    if (isset($_SESSION['roll']) && isset($_SESSION['name']) && isset($_SESSION['cpi']) && isset($_SESSION['backlogs'])) {
        // Retrieve session variables
        $roll = $_SESSION['roll'];
        $name = $_SESSION['name'];
        $cpi = $_SESSION['cpi'];
        $backlogs = $_SESSION['backlogs'];
        ...
        // Prepare and execute the SQL query to fetch eligible jobs
        $sql_query = "SELECT * FROM jobs WHERE MinCPI <= $cpi and ((allow_backlogs = 'YES') or (allow_backlogs = '$backlogs')) ";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            while ($job = mysqli_fetch_assoc($result)) {
                ...
                // Prepare the action column based on whether the student has applied
                if ($apply_check_result && mysqli_num_rows($apply_check_result) > 0) {
                    // Student has already applied
                    $action = "Already Applied";
                } else {
                    // Student has not applied, create an apply link
                    $action = "<a href='application.php?companyID=" . urlencode($job['CompanyID']) . 
                              "&companyName=" . urlencode($job['CompanyName']) . 
                              "&position=" . urlencode($job['Position']) . 
                              "&JobID=" . urlencode($jobID) . 
                              "&jobDescription=" . urlencode($job['JobDescription']) . 
                              "&salary=" . urlencode($job['Salary']) . 
                              "&allow_backlogs=" .urlencode(string:$job['Allow_Backlogs']).
                              "&minCPI=" . urlencode($job['MinCPI']) . 
                              "'>Apply</a>";
                }
            ...
            echo "<p>No eligible jobs found.</p>";
        }
